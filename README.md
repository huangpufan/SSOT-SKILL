...
**Have an agent? Paste this into your repo's chat:**

> Install the SSOT Skill bundle from `https://github.com/huangpufan/SSOT-SKILL` into this project using the one-liner in its README, then read `AGENTS.md`.

The agent runs the installer, reads `AGENTS.md`, and bootstraps `SSOT/` for you.
...
The installer opens a picker for agent, scope, and template language. Restart your agent session, then run `$ssot-bootstrap` in your repo to create `SSOT/` and wire the adapter.
...
Pass the same agent target you installed into (e.g. `claude-code`, `codex`). The installer also supports `--upgrade` and `--version`.
...
```
┌─────────────────────────┐    install.sh    ┌──────────────────────────┐
│  SSOT-SKILL (this repo) │ ───────────────▶ │  .claude/skills/...      │  ← agent loads
│  installer · 6 skills   │                  │  .codex/skills/...       │     these at startup
