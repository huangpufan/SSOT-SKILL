...
This file is the semantic owner of source material classification, absorption status, thin-documentation rules, conflict adjudication, and product and architecture source routing. `STATUS.md` only maintains the matrix and tracking-baseline fields; this file maintains how to judge and process source material.
...
| User-visible accessibility, locale/time-zone/format behaviour, usability, performance expectation, availability/degradation, compatibility, data handling, notification, consent, licensing, safety, oversight, fairness, transparency, commercial entitlement, paid action, output validity, or disclosure promise | The relevant `01-product/` model, capability, journey, or acceptance owner; link the matching architecture/process owner or named `Q01`-`Q21` gap instead of copying its mechanism |
...
| Runtime topology, deployment units or variants, environment/configuration differences, health/readiness, log/metric/trace meaning, alert-to-diagnosis, restart, rollback, and recovery proof | `02-architecture/views/deployment-and-observability.md`, linking `03-process/deployment/` or conditional `03-process/operations/` for the executable procedure |
...
| Script/tool inventory, build/test/benchmark commands, model generation, session analysis, version sync, import rewriting and other engineering automation | Default route to `03-process/development/`, `03-process/testing/`, `03-process/benchmark/`, `03-process/release/`, `03-process/deployment/`, or applicable `03-process/operations/` / `03-process/security-and-compliance/`; only when a script carries model-generation pipeline, session analysis, release consistency, state migration, or other architecture behavior does it also enter the relevant architecture view/domain |
| Repeatable operator runbook, maintenance, incident triage, quota/capacity response, backup/restore drill, continuity exercise, or live recovery handoff | Conditional `03-process/operations/`, linking deployment and architecture signal/state owners |
| Recurring security/privacy/compliance control, access/vulnerability review, audit-evidence collection, signing/SBOM/license procedure, security incident handoff, or notification duty | Conditional `03-process/security-and-compliance/`, linking the product promise and architecture trust/threat owner |
